...
int COPY(char *path1, char *path2)
{
    ...
    printf("PPP%d %d\n", ss_struct_1->port_no_ns, ss_struct_2->port_no_ns);
    // for file
...
int search_for_ss(int port){
    ...
    for(int i = 0; i < ss_num; i++){
        // printf("%d\n", ss_array[i].port_no_ns);
        if(ss_array[i].port_no_ns == port){
            return i;
        }
    }
    return -1;
...
void *ss_handler(void *param)
{
    ...
    if(ss_index == -1 || backup_ss_array[ss_index].has_dup == 0){
        ...
        for(int i = ss_num; i >= 0; i--){
            printf("X%d %d %d\n", ss_index, ss_array[i].port_no_ns, ss_num);
            if(ss_array[i].port_no_ns != ss_array[ss_index].port_no_ns && ss_array[i].port_no_ns != 0){
                printf("AD %d\n", i);
        ...
        if(found == 2){
            ...
            strcpy(backup_ss_array[ss_index].ip_addr_1, ss_array[backups[0]].ip_addr);
            strcpy(backup_ss_array[ss_index].ip_addr_2, ss_array[backups[1]].ip_addr);
    ...
    for (int k = 0; k < ss_array[ss_index].no_acc_paths; k++)
    {
        char buffer[1024];
        printf("%s\n", ss_array[ss_index].accesible_paths[k]); 
        if (indxx){
            insert(root, ss_array[ss_index].accesible_paths[k],&ss_array[ss_index]);
        }
        if(found == 2){
