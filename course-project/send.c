...
int execute(char *command, int *client_socket, struct LRUcache *lruQueue, struct TrieNode *root)
{
    ...
    if (strcmp(token, "COPY") == 0)
    {
        ...
        logMessage(command, *client_socket, 1);
    ...
    else
    {
        ...
        comm = strtok(comm, "\t\n");
        printf("sfdgf\n");
        int index = search_for_ss(ss_struct->port_no_ns);

        ss_info* ss_struct_b1;
        ss_info* ss_struct_b2;

        printf("AFSG %d %d %d %d\n", flag, backup_ss_array[index].has_dup, index, ss_struct->port_no_ns);
        if(backup_ss_array[index].has_dup){
            // snprintf(buffer, sizeof(buffer), "backup/SS%d_1/%s", ss_struct->port_no_ns, token);
            ss_struct_b1 = &ss_array[search_for_ss(backup_ss_array[index].port_no_ns_b1)];

            // snprintf(buffer, sizeof(buffer), "backup/SS%d_2/%s", ss_struct->port_no_ns, token);
            ss_struct_b2 = &ss_array[search_for_ss(backup_ss_array[index].port_no_ns_b2)];
            printf("afa\n");
        }
        if (flag == 0 && backup_ss_array[index].has_dup && strcmp(comm, "WRITE") != 0)
        {
            char buffer[MAX_CHARS * 3];
        ...
        else if (flag == 1 && strcmp(comm, "WRITE") == 0)
        {
            if (backup_ss_array[index].has_dup && ss_struct_b1 != NULL && ss_struct_b2 != NULL)
            {
